...
-- ==========================================
-- 1. АРХІВАЦІЯ СТАРОЇ ТАБЛИЦІ
-- ==========================================
ALTER TABLE IF EXISTS public.audit_event RENAME TO audit_event_archive;

-- Звільняємо імена констрейнтів
ALTER TABLE public.audit_event_archive RENAME CONSTRAINT audit_event__id__pk TO audit_event_archive__id__pk;
ALTER TABLE public.audit_event_archive RENAME CONSTRAINT audit_event__type__ck TO audit_event_archive__type__ck;

-- 2. СТВОРЕННЯ МАЙСТЕР-ТАБЛИЦІ
...
-- 3. СТВОРЕННЯ VIEW ДЛЯ KAFKA
CREATE VIEW public.audit_event AS
SELECT * FROM public.audit_event_master;

-- 4. ПРАВА ДОСТУПУ
ALTER TABLE IF EXISTS public.audit_event_master OWNER to postgres;
ALTER VIEW IF EXISTS public.audit_event OWNER to postgres;

-- Кафці потрібен лише SELECT (для метаданих View) та INSERT
